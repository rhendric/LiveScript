...
(Node = function(){
  ...
  toString: function(idt){
    ...
    if (this.typeAscription) {
      tree += '\n' + idt + ':: ' + this.typeAscription.toString(idt + TAB);
    }
    return tree;
  },
  ...
  addType: function(typeAscription){
    this.typeAscription = typeAscription;
    return this;
  }
};
...
exports.Block = Block = (function(superclass){
  ...
  Block.prototype.compileRoot = function(options){
    ...
    return result = sn.apply(null, [null, prefix || []].concat(arrayFrom$(code)));
  };
...
exports.Chain = Chain = (function(superclass){
  ...
  Chain.prototype.compileNode = function(o){
    var head, tails, typeParams, typeBody, ref$, p, that, i$, len$, t, hasPartial, pre, rest, broken, partial, post, context, idt, func, base, news;
    if (this.flip) {
      util('flip');
      util('curry');
    }
    head = this.head, tails = this.tails;
    if (o.inTypeExpr && head.value === 'forall') {
      typeParams = tails[0].args.slice(0, -1);
      typeBody = (ref$ = tails[0].args)[ref$.length - 1];
      return sn(this, '<', (function(){
        var i$, ref$, len$, results$ = [];
        for (i$ = 0, len$ = (ref$ = typeParams).length; i$ < len$; ++i$) {
          p = ref$[i$];
          results$.push(p.compile(o));
        }
        return results$;
      }()).join(', '), '>', typeBody.compile(o));
    }
...
exports.Call = Call = (function(superclass){
  ...
  Call.prototype.compile = function(o){
    ...
    code = [sn(this, this.method || '', o.inTypeExpr ? '<' : '(') + (this.pipe ? "\n" + o.indent : '')];
    for (i$ = 0, len$ = (ref$ = this.args).length; i$ < len$; ++i$) {
      i = i$;
      a = ref$[i$];
      code.push(i ? ', ' : '', a.compile(o, LEVEL_LIST));
    }
    code.push(sn(this, o.inTypeExpr ? '>' : ')'));
    return sn.apply(null, [null].concat(arrayFrom$(code)));
  };
...
exports.Assign = Assign = (function(superclass){
  ...
  Assign.prototype.compileNode = function(o){
    var left, sp, ref$, i$, len$, op, right, reft, sign, name, lvar, anyAscriptions, typeAscription, param, that, ref1$, protoSplit, dotSplit, code, empty, res;
    ...
    if (lvar = left instanceof Var) {
      if (op === '=') {
        ...
        if (right instanceof Fun && !right.typeAscription) {
          anyAscriptions = false;
          typeAscription = Fun();
          for (i$ = 0, len$ = (ref$ = right.params).length; i$ < len$; ++i$) {
            param = ref$[i$];
            typeAscription.params.push((that = (ref1$ = param.typeAscription, delete param.typeAscription, ref1$))
              ? (anyAscriptions = true, that)
              : Var('any'));
          }
          typeAscription.body = Block((that = (ref$ = (ref1$ = right.body.lines)[ref1$.length - 1]) != null ? (ref1$ = ref$.typeAscription, delete ref$.typeAscription, ref1$) : void 8)
            ? (anyAscriptions = true, that)
            : Var('any'));
          if (anyAscriptions) {
            right.typeAscription = typeAscription;
          }
        }
        if (that = (ref1$ = right.typeAscription, delete right.typeAscription, ref1$)) {
          o.scope.ascribe(name, that.compile(import$(clone$(o), typeO)));
        }
...
exports.Fun = Fun = (function(superclass){
  ...
  Fun.prototype.compileNode = function(o){
    var n, p, forallArgs, ref$, ref1$, pscope, sscope, scope, that, inLoop, body, name, tab, code, returnType, ref2$, ref3$, i$, x$, len$, bodyCode, curryCodeCheck, this$ = this;
    if (o.inTypeExpr) {
      n = 0;
      return sn(null, '(', (function(){
        var i$, ref$, len$, results$ = [];
        for (i$ = 0, len$ = (ref$ = this.params).length; i$ < len$; ++i$) {
          p = ref$[i$];
          results$.push("arg" + (n++) + ": " + p.compile(o));
        }
        return results$;
      }.call(this)).join(', '), ') => ', this.body.lines[0].compile(o));
    }
    if (this.typeAscription instanceof Chain && this.typeAscription.head.value === 'forall') {
      forallArgs = this.typeAscription.tails[0].args;
      this.typeAscriptionGenerics = forallArgs.slice(0, -1);
      this.typeAscription = forallArgs[forallArgs.length - 1];
    }
    if (this.typeAscription instanceof Fun) {
      zipWith(function(p, t){
        return p.addType(t);
      }, this.params, this.typeAscription.params);
      if ((ref$ = (ref1$ = this.body.lines)[ref1$.length - 1]) != null) {
        ref$.addType(this.typeAscription.body.lines[0]);
      }
    }
    pscope = o.scope;
    ...
    if (inLoop = (ref1$ = o.loop, delete o.loop, ref1$)) {
      o.indent = this.tab = '';
    }
    o.indent += TAB;
    ...
    if (this.async) {
      ...
      this.generator && this.carp("a generator can't be async");
      o.inAsync = true;
      code.unshift('async ');
    ...
    returnType = (ref2$ = (ref3$ = body.lines)[ref3$.length - 1]) != null ? ref2$.typeAscription : void 8;
    this.hushed || this.ctor || this.newed || body.makeReturn();
    if (that = this.typeAscriptionGenerics) {
      code.push("<");
      for (i$ = 0, len$ = that.length; i$ < len$; ++i$) {
        x$ = that[i$];
        code.push(x$.value, ', ');
    ...
    if (that = !(this.hushed || this.ctor || this.newed) && returnType) {
      code.push(': ', that.compile(import$(clone$(o), typeO)));
    }
  ...
  Fun.prototype.compileParams = function(o, scope){
    ...
    for (i$ = 0, len$ = params.length; i$ < len$; ++i$) {
      ...
      names.push.apply(names, [scope.add(vr.value, 'arg', p)].concat(
        (that = vr.typeAscription)
          ? [': ', that.compile(import$(clone$(o), typeO))]
          : [], [', ']
      ));
    }
    if (rest) {
...
exports.For = For = (function(superclass){
  ...
  For.prototype.addBody = function(body){
    var hasYield, ref$, that, assignments, x$, assigned, name, ref1$;
    ...
    if (this['let']) {
      ...
      body = Block(this.guard
        ? (assigned = (function(){
          ...
          return results$;
        }()), assignments.concat([If((ref1$ = this.guard, delete this.guard, ref1$), Call['let'](assigned, body, hasYield))]))
        : Call['let'](assignments, body, hasYield));
    }
...
ref$.add = function(name, type, node){
  var t, ref$, that;
  if (node && (t = (ref$ = this.variables[name + "."]) != null ? ref$.type : void 8)) {
...
ref$.get = function(name){
  var ref$;
  return (ref$ = this.variables[name + "."]) != null ? ref$.type : void 8;
};
...
ref$.temporary = function(name){
  var ref$, ref1$;
  name || (name = 'ref');
  while ((ref$ = (ref1$ = this.variables[name + "$."]) != null ? ref1$.type : void 8) !== 'reuse' && ref$ !== void 8) {
...
ref$.check = function(name, above){
  var type, ref$, ref1$;
  if ((type = (ref$ = this.variables[name + "."]) != null ? ref$.type : void 8) || !above) {
    return type;
  }
  return (ref1$ = this.parent) != null ? ref1$.check(name, above) : void 8;
};
ref$.checkReadOnly = function(name){
  ...
  (ref$ = this.variables)[key$ = name + "."] || (ref$[key$] = {
    type: 'upvar'
  });
  return '';
};
ref$.ascribe = function(name, ascription){
  var ref$;
  return ref$ = this.variables[name + "."], ref$.ascription = ascription, ref$;
};
ref$.emit = function(code, tab){
  var vrs, asn, fun, name, ref$, x, type, ascription, that, val, declCode;
  vrs = [];
  asn = [];
  fun = [];
  for (name in ref$ = this.variables) {
    x = ref$[name], type = x.type, ascription = x.ascription;
    name = name.slice(0, -1);
...
typeScope = new Scope;
for (fn in Scope.prototype) {
  typeScope[fn] = fn$;
}
typeScope.add = function(it){
  return it;
};
typeO = {
  inTypeExpr: true,
