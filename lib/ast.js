...
(Node = function(){
  ...
  deleteType: function(){
    var ref$;
    return ref$ = this.typeAscription, delete this.typeAscription, ref$;
  }
};
...
exports.Block = Block = (function(superclass){
  ...
  Block.prototype.deleteType = function(){
    var ref$, ref1$;
    return (ref$ = this.typeAscription, delete this.typeAscription, ref$) || ((ref$ = (ref1$ = this.lines)[ref1$.length - 1]) != null ? ref$.deleteType() : void 8);
  };
  return Block;
}(Node));
...
exports.Fun = Fun = (function(superclass){
  ...
  Fun.prototype.compileNode = function(o){
    var n, p, type, forallArgs, typeAscriptionGenerics, returnType, pscope, sscope, scope, that, inLoop, ref$, body, name, tab, code, i$, x$, len$, bodyCode, curryCodeCheck, this$ = this;
    if (o.inTypeExpr) {
      n = 0;
    ...
    if (type instanceof Chain && type.head.value === 'forall') {
      forallArgs = type.tails[0].args;
      typeAscriptionGenerics = forallArgs.slice(0, -1);
      type = forallArgs[forallArgs.length - 1];
    }
    if (type instanceof Fun) {
      zipWith(function(p, t){
        return p.addType(t);
      }, this.params, type.params);
      returnType = type.body.lines[0];
    }
    pscope = o.scope;
    ...
    if (that = typeAscriptionGenerics) {
      code.push("<");
      for (i$ = 0, len$ = that.length; i$ < len$; ++i$) {
        x$ = that[i$];
        code.push(x$.value, ', ');
  ...
  Fun.prototype.deleteType = function(){
    var ref$, anyAscriptions, typeAscription, param, that;
    return (ref$ = this.typeAscription, delete this.typeAscription, ref$) || (anyAscriptions = false, typeAscription = Fun(), (function(){
      var i$, ref$, len$, results$ = [];
      for (i$ = 0, len$ = (ref$ = this.params).length; i$ < len$; ++i$) {
        param = ref$[i$];
        results$.push(typeAscription.params.push((that = param.deleteType())
          ? (anyAscriptions = true, that)
          : Var('any')));
      }
      return results$;
    }.call(this)), typeAscription.body = Block((that = this.body.deleteType())
      ? (anyAscriptions = true, that)
      : Var('any')), anyAscriptions ? typeAscription : void 8);
  };
  return Fun;
}(Node));
