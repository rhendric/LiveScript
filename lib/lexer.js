...
exports.doLiteral = function(code, index){
  ...
  case '~':
    ...
    break;
  case '::':
    if (this.last.spaced || ((ref$ = this.last[0]) === 'DEDENT' || ref$ === 'INDENT' || ref$ === 'NEWLINE')) {
      tag = 'ASCR';
    } else {
      this.adi();
      val = 'prototype';
...
function firstPass(tokens){
  ...
  while (token = tokens[++i]) {
    ...
    switch (false) {
    case !(tag === 'ASCR' && !(token.spaced && (next = tokens[i + 1]) && (next[0] === 'PARAM(' || in$(next[0], CHAIN)))):
      token[0] = 'ID';
      token[1] = 'prototype';
      break;
